...
class OxygenSystem:
    ...
    def __init__(
        ...
        resolved_image_path = image_path if os.path.isabs(image_path) else os.path.join(BASE_DIR, image_path)
        raw_image = pygame.image.load(resolved_image_path).convert_alpha()
        self.image = pygame.transform.smoothscale(raw_image, tank_size)

        self._collect_sound = pygame.mixer.Sound(os.path.join(AUDIO_DIR, "oxygen tank.ogg"))
